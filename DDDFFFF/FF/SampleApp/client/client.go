	...
	for name, svcProps := range props.Services {
		...
		if cbCfg.Enabled {
			st := gobreaker.Settings{
				Name:        name,
				MaxRequests: cbCfg.HalfOpenMaxRequests, // fixed: used for half-open state
				Timeout:     time.Duration(cbCfg.WaitDurationInOpenStateMs) * time.Millisecond,
				ReadyToTrip: func(counts gobreaker.Counts) bool {
					if counts.Requests < cbCfg.MinimumNumberOfCalls {
						return false
					}
					failureRatio := float64(counts.TotalFailures) / float64(counts.Requests)
					return failureRatio >= (float64(cbCfg.FailureRateThreshold) / 100.0)
				},
			}
