		...
		if cbCfg.Enabled {
			st := gobreaker.Settings{
				...
				ReadyToTrip: func(counts gobreaker.Counts) bool {
					...
					return counts.Requests >= cbCfg.MinimumNumberOfCalls && failureRatio >= (float64(cbCfg.FailureRateThreshold)/100.0)
				},
			}
...
func (wc *WebClient) DoGet(path string) ([]byte, error) {
	fmt.Println("This IS FROM CLIENT DO GET Line Number 75")
