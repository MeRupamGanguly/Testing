...
docker stop "$CONTAINER_NAME" "$REDIS_CONTAINER" "$JAEGER_CONTAINER" 2>/dev/null || true
docker rm "$CONTAINER_NAME" "$REDIS_CONTAINER" "$JAEGER_CONTAINER" 2>/dev/null || true
...
if ! command -v jq &> /dev/null; then
    echo -e "${YELLOW}jq not found. Install with: sudo apt install jq   (or use your package manager)${NC}"
...
docker run -d --name "$JAEGER_CONTAINER" --network "$NETWORK_NAME" \
