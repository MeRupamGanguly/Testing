...
pkill -f "podman logs -f $CONTAINER_NAME" 2>/dev/null
...
podman logs -f $CONTAINER_NAME 2>&1 | tee "$LOG_FILE" &
LOG_PID=$!
...
NEW_VAL=$(podman exec $CONTAINER_NAME sh -c "grep -A5 '\"log-levels\"' /app/flags.json | grep fallthrough")
