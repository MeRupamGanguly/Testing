...
var MaxResponseSize int64 = 1 << 20

func SetMaxResponseSize(size int64) {
	if size > 0 {
		MaxResponseSize = size
	}
}

func safeJSON(c *gin.Context, code int, envelope Envelope) {
	data, err := json.Marshal(envelope)
	if err != nil {
		c.String(500, `{"success":false,"error":{"code":"INTERNAL_ERROR","message":"marshal failed"}}`)
		return
	}
	if int64(len(data)) > MaxResponseSize {
		fallback := Envelope{
			Success: false,
			Error:   &ErrorDetail{Code: "RESPONSE_TOO_LARGE", Message: "response payload too large"},
		}
		fb, _ := json.Marshal(fallback)
		c.Header("Content-Type", "application/json")
		c.String(500, string(fb))
		return
	}
